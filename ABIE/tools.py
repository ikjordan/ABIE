...
class Tools(object):
    ...
    def from_orbital_elements_to_cartesian(
        ...
        e_vec_x = cos_long_asc_nodes * cos_arg_per - sin_long_asc_nodes * sin_arg_per * cos_inclination
        e_vec_y = sin_long_asc_nodes * cos_arg_per + cos_long_asc_nodes * sin_arg_per * cos_inclination
        ...
        q_vec_x = -cos_long_asc_nodes * sin_arg_per - sin_long_asc_nodes * cos_arg_per * cos_inclination
        q_vec_y = -sin_long_asc_nodes * sin_arg_per + cos_long_asc_nodes * cos_arg_per * cos_inclination
        ...
        separation = semimajor_axis * (1.0 - eccentricity ** 2) / (
                     1.0 + eccentricity * cos_true_anomaly)  # Compute the relative separation
        position_vector = separation * cos_true_anomaly * e_vec + separation * sin_true_anomaly * q_vec
        velocity_tilde = np.sqrt(G * (mp + ms) / (semimajor_axis * (1.0 - eccentricity ** 2)))  # Common factor
        velocity_vector = -1.0 * velocity_tilde * sin_true_anomaly * e_vec + velocity_tilde * (
                          eccentricity + cos_true_anomaly) * q_vec

        return position_vector, velocity_vector

    @staticmethod
    def from_cartesian_to_orbital_elements(mp, ms, position, velocity, G=4.0 * np.pi ** 2):
        """
        ...
        specific_angular_momentum_norm = np.sqrt(np.dot(specific_angular_momentum, specific_angular_momentum))
        specific_angular_momentum_unit = specific_angular_momentum / specific_angular_momentum_norm

        maximum_specific_angular_momentum_norm = G * total_mass / (np.sqrt(-2.0 * specific_energy))
        ...
        ascending_node_vector_norm = np.sqrt(np.dot(ascending_node_vector, ascending_node_vector))
        ...
        long_asc_nodes = np.arctan2(ascending_node_vector_unit[1], ascending_node_vector_unit[0])

        ### Argument of periapsis and true anomaly, using eccentricity a.k.a. Laplace-Runge-Lenz (LRL) vector ###
        mu = G * total_mass
        position_unit = position / r
        e_vector = (1.0 / mu) * np.cross(velocity,
                                            specific_angular_momentum) - position_unit  ### Laplace-Runge-Lenz vector
        ...
        e_vector_unit_cross_AM_unit = np.cross(e_vector_unit, specific_angular_momentum_unit)
        ...
        return semimajor_axis, eccentricity, true_anomaly, inclination, arg_per, long_asc_nodes

    @staticmethod
    def from_cartesian_to_aei(mp, ms, position, velocity, G=0.0172020989**2, z_vector=np.array([0., 0., 1.])):
        """
        ...
        if velocity.ndim == 1 and position.ndim == 1:
            ...
            specific_angular_momentum_norm = np.sqrt(np.dot(specific_angular_momentum, specific_angular_momentum))
            specific_angular_momentum_unit = specific_angular_momentum / specific_angular_momentum_norm
            ecc_vec = ((v_sq - mu / r) * position - np.dot(position, velocity) * velocity) / mu
            ...
            ### specific angular momentum ###
            specific_angular_momentum = np.cross(position.transpose(), velocity.transpose()).transpose()
            specific_angular_momentum_norm = np.sqrt(
                np.sum(specific_angular_momentum * specific_angular_momentum, axis=0))
            specific_angular_momentum_unit = specific_angular_momentum / specific_angular_momentum_norm
            ecc_vec = ((v_sq - mu / r) * position - np.sum(position * velocity, axis=0) * velocity) / mu
