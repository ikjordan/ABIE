...
def display_3d_data(input_file, hash2names=None, title="Trajectory", scatter=False):
    # flatten the data
...
def display_2d_data(input_file, hash2names=None, title="Trajectory", scatter=False):
    # flatten the data
    converted = snapshot_convert(input_file)
    ...
    if converted:
        ...
        for i in range(0, len(x[0])): 
            ...
            if scatter:
                ax.scatter(x[:,i], y[:,i], s=0.2, label=names[i] if hash2names is not None else None)
            else:
                ax.plot(x[:,i], y[:,i], label=names[i] if hash2names is not None else None)
